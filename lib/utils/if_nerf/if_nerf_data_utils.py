...
def get_rays(H, W, K, R, T):
    # calculate the camera origin，计算相机原点，即光线原点
    rays_o = -np.dot(R.T, T).ravel()
    # calculate the world coodinates of pixels，计算像素的世界坐标
    ...
    # calculate the ray direction，计算光线方向
...
def get_bound_corners(bounds):  # 根据3D边界框的最小顶点和最大顶点获取边界框的8个顶点
...
def get_bound_2d_mask(bounds, K, pose, H, W):
    corners_3d = get_bound_corners(bounds)  # 获取3d边界框的8个顶点
    corners_2d = base_utils.project(corners_3d, K, pose)  # 投影3d边界框到2d平面
    corners_2d = np.round(corners_2d).astype(int)  # round函数表示四舍五入取整
    mask = np.zeros((H, W), dtype=np.uint8)  # 定义一个全零矩阵
...
def get_near_far(bounds, ray_o, ray_d):
    """calculate intersections with 3d bounding box，计算与三维边界框的交点"""
    bounds = bounds + np.array([-0.01, 0.01])[:, None]  # [:, None]等效于reshape(1, -1)
    nominator = bounds[None] - ray_o[:, None]  # [None]可用于扩展维数，如维数[1, 2]——>[1, 1, 2]
    ...
    min_x, min_y, min_z, max_x, max_y, max_z = bounds.ravel()  # 获取3d边界框的最小顶点和最大顶点的值
    eps = 1e-6
...
def sample_ray_h36m(img, msk, K, R, T, bounds, nrays, split):
    ...
    ray_o, ray_d = get_rays(H, W, K, R, T)  # 获取相机射线

    pose = np.concatenate([R, T], axis=1)  # 合并旋转向量和平移向量
    bound_mask = get_bound_2d_mask(bounds, K, pose, H, W)  # 获取3d边界框的2d投影框，K为相机参数

    if cfg.mask_bkgd:
        img[bound_mask != 1] = 0  # 将图片背景像素设置为0

    msk = msk * bound_mask  # 获取新的msk
    bound_mask[msk == 100] = 0

    if split == 'train':  # 对于训练集
        nsampled_rays = 0
        ...
        rgb = img.reshape(-1, 3).astype(np.float32)  # 设置img形状及数据类型
        ray_o = ray_o.reshape(-1, 3).astype(np.float32)  # 设置ray_o形状及数据类型
        ray_d = ray_d.reshape(-1, 3).astype(np.float32)  # 设置ray_d形状及数据类型
        near, far, mask_at_box = get_near_far(bounds, ray_o, ray_d)  # 获取远近界及box处的遮罩
        near = near.astype(np.float32)
        far = far.astype(np.float32)
        rgb = rgb[mask_at_box]
        ray_o = ray_o[mask_at_box]
...
def get_rigid_transformation(poses, joints, parents, return_joints=False):
    ...
    parents: 24
    """
    rot_mats = batch_rodrigues(poses)  # 把旋转向量转化为旋转矩阵

    # obtain the relative joints，获取相对关节
    rel_joints = joints.copy()
    rel_joints[1:] -= joints[parents[1:]]

    # create the transformation matrix，创建一个变换矩阵
    transforms_mat = np.concatenate([rot_mats, rel_joints[..., None]], axis=2)
    padding = np.zeros([24, 1, 4])  # 定义一个指定形状的数组
    padding[..., 3] = 1  # 设置最后一个维度数组的第3索引元素的值为1
    transforms_mat = np.concatenate([transforms_mat, padding], axis=1)

    # rotate each part，旋转每个部分
    ...
    # obtain the rigid transformation，获取刚性变换矩阵
...
def get_bounds(xyz):
    min_xyz = np.min(xyz, axis=0)  # 获取3d边界框的最小顶点
    max_xyz = np.max(xyz, axis=0)  # 获取3d边界框的最大顶点
    min_xyz -= cfg.box_padding  # 根据边界框偏移量修改最小顶点
    max_xyz += cfg.box_padding  # 根据边界框偏移量修改最大顶点
    bounds = np.stack([min_xyz, max_xyz], axis=0)
    bounds = bounds.astype(np.float32)  # 设置数据类型
    return bounds
