...
def pose_points_to_tpose_points(ppts, bw, A):
    ...
    sh = ppts.shape
    bw = bw.permute(0, 2, 1)
    # ma: [a, b, c]；mb: [a, c, d]；torch.bmm(ma, mb)——>[a, b, d]；对应公式中的w(x)*G
    A = torch.bmm(bw, A.view(sh[0], 24, -1))
    A = A.view(sh[0], -1, 4, 4)  # 改变张量形状
    pts = ppts - A[..., :3, 3]
    R_inv = torch.inverse(A[..., :3, :3])  # 求矩阵的逆
    pts = torch.sum(R_inv * pts[:, :, None], dim=3)  # 对应公式4
    return pts


def pose_dirs_to_tpose_dirs(ddirs, bw, A):
    """transform directions from the pose space to the T pose，将姿势空间的方向转换到tpose空间
...
def tpose_points_to_pose_points(pts, bw, A):
    """transform points from the T pose to the pose space，将tpose空间的点转换到姿势空间
    ppts: n_batch, n_points, 3
    bw: n_batch, 24, n_points
    A: n_batch, 24, 4, 4
    """
...
def pts_sample_blend_weights(pts, bw, bounds):
    ...
    bw: n_batch, d, h, w, 25【由数据集提供】
    bounds: n_batch, 2, 3
    """
    pts = pts.clone()

    # interpolate blend weights，插值混合权重
    min_xyz = bounds[:, 0]  # 获取近界
    max_xyz = bounds[:, 1]  # 获取远界
    bounds = max_xyz[:, None] - min_xyz[:, None]  # 获取边界
    grid_coords = (pts - min_xyz[:, None]) / bounds
    grid_coords = grid_coords * 2 - 1
    # convert xyz to zyx, since the blend weight is indexed by xyz，将xyz转换为zyx，因为混合权重由xyz索引
