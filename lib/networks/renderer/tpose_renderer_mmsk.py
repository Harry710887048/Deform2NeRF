...
class Renderer(tpose_renderer.Renderer):
    ...
    def get_density_color(self, wpts, viewdir, z_vals, inside, raw_decoder):
        ...
        full_raw = torch.zeros([n_batch * n_pixel * n_sample, 4]).to(wpts)  # 定义一个指定大小的张量，并使其类型与wpts一致
        if inside.sum() == 0:
            ret = {'raw': full_raw}
            return ret
        ...
        full_raw[inside] = ret['raw']  # 获取network输出值中的raw，即tpose空间采样点的颜色和密度数据
        ret = {'raw': full_raw}

        return ret  # 然后rbg、alpha数据

    def get_pixel_value(self, ray_o, ray_d, near, far, batch):
        n_batch = ray_o.shape[0]

        # sampling points for nerf training，获取nerf训练的采样点
        ...
        # compute the color and density，计算采样点颜色和密度
