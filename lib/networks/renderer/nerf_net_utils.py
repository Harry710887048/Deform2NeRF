...
def raw2outputs(raw, z_vals, white_bkgd=False):
    ...
    """
    rgb = raw[..., :-1]  # [N_rays, N_samples, 3]，采样点的rgb值集合
    alpha = raw[..., -1]  # 采样点的密度值集合

    # weights = alpha * tf.math.cumprod(1.-alpha + 1e-10, -1, exclusive=True)
    # cumprod表示逐级累加，如[1,2,3,4]——>[1,3,6,10]
    weights = alpha * torch.cumprod(
        torch.cat(
            [torch.ones((alpha.shape[0], 1)).to(alpha), 1. - alpha + 1e-10],
            -1), -1)[:, :-1]  # 计算体积渲染方程中的T，表示指定给每个采样点颜色的权重
    rgb_map = torch.sum(weights[..., None] * rgb, -2)  # [N_rays, 3]【射线颜色】，对应原始NeRF论文中的公式5

    depth_map = torch.sum(weights * z_vals, -1)  # 估计到物体的距离
    disp_map = 1. / torch.max(1e-10 * torch.ones_like(depth_map).to(depth_map),
                              depth_map / torch.sum(weights, -1))  # 视差图。1/深度
    acc_map = torch.sum(weights, -1)  # 沿每条射线的权重总和

    if white_bkgd:  # 处理白色背景
